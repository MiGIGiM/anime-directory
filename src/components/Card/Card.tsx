...
const Card: FC<{ anime: Anime }> = ({ anime }) => {
    const getAiring = (): string => {
        ...
        if (anime.type !== 'TV') return start.toLocaleDateString('en-US');
                ...
                <div className={style.flipContent}>
                    ...
                    <div className={style.flipBack}>
                        <div className="w-full h-full bg-gray-400 p-2 space-y-8 relative">
                            ...
                            <span className="flex items-baseline flex-row-reverse justify-center flex-wrap absolute bottom-2">
                                {anime.genres.map((genre) => (
                                    <p
                                        key={genre.mal_id}
                                        className="bg-white rounded-full px-3 py-1 text-[10px] whitespace-nowrap text-gray-700 m-1"
                                    >
                                        {genre.name}
                                    </p>
                                ))}
